...
- Import and recognition are separate: the import dialog only saves one or more PDFs to the local library and never displays OCR progress. Saved documents enter a durable serial background queue so additional PDFs can be imported while another document is recognizing.
- Background recognition does not use a floating progress panel. Its library row owns status and controls: an active or queued job shows a stop button, and a paused job shows a resume button.
...
- OPFS source blobs are rewrapped with the persisted original filename when loaded. Resumed processing preserves immutable `originalTitle` and `originalFileName` metadata instead of deriving identity from the internal `source.pdf` storage filename.
...
- Saved PDFs are reopened from binary data rather than Blob URLs or custom range transports. PDF.js transfers that buffer to its worker, avoiding WebKit network and structured-clone failures after restarting an installed app.
